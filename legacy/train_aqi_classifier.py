...
def stage1_training():
    ...
    print("\n=== STEP 1: APPLY SMOTE ===")
    sm = SMOTE(sampling_strategy='not majority', random_state=42, k_neighbors=3)
    ...
    baseline_acc = accuracy_score(y_test, xgb_b.predict(X_test))
    ...
    xgb_3.fit(X_train_st, y_res)
    acc_3 = accuracy_score(y_test, xgb_3.predict(X_test_st))
    ...
    xgb_e = XGBClassifier(n_estimators=500, max_depth=10, learning_rate=0.08, subsample=0.9, colsample_bytree=0.9,
                          min_child_weight=1, gamma=0.1, reg_alpha=0.01, reg_lambda=0.01,
                          tree_method='hist', device='cpu', use_label_encoder=False, eval_metric='mlogloss',
                          random_state=42, n_jobs=-1, verbosity=0)
    xgb_e.fit(current_X_train, y_res)
    acc_xgb = accuracy_score(y_test, xgb_e.predict(current_X_test))
    
    lgb_e = LGBMClassifier(n_estimators=500, max_depth=10, learning_rate=0.08, num_leaves=127,
                           lambda_l1=0.01, lambda_l2=0.01,
                           random_state=42, n_jobs=-1, verbosity=-1)
    lgb_e.fit(current_X_train, y_res)
    acc_lgb = accuracy_score(y_test, lgb_e.predict(current_X_test))
    
    rf_e = RandomForestClassifier(n_estimators=300, max_depth=25, min_samples_split=2, min_samples_leaf=1,
                                  random_state=42, n_jobs=-1)
    rf_e.fit(current_X_train, y_res)
    acc_rf = accuracy_score(y_test, rf_e.predict(current_X_test))
    ...
    acc_ens = accuracy_score(y_test, ensemble.predict(current_X_test))
    ...
    if current_acc < 0.97:
        ...
        def objective(trial):
            ...
            return accuracy_score(y_test, xgb.predict(current_X_test))
